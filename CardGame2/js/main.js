...
function createDeck(){
    ...
    if(exDeckList.length > 0){
        const exDeck = document.getElementById("exDeck");
        exDeck.style.display = "block";
        const exDeckText = document.getElementById("exDeck-text");
        exDeckText.textContent = `${exDeckList.length}枚`;
    } else {
        const exDeck = document.getElementById("exDeck");
        exDeck.style.display = "none";
    }
...
function drawDeck(){
    ...
    cardImg.addEventListener("mousedown", mdown, false);
    card_place.appendChild(cardImg);
...
function setOtherCard(){
    for(let i=0;i<otherList.length;i++){
        const card_place = document.getElementById("card_place2");
